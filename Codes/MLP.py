...
Accuracy: 0.45182
"""
model = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(100,), random_state=1).fit(X_train, y_train)
...
test_raw.to_csv('Test_MLP.csv', index=False)

val_raw = pd.read_csv('Validate.csv')
val_raw["Pred"] = predictions
val_raw.to_csv('Validate_MLP.csv', index=False)
train_raw = pd.read_csv('Train.csv')
pred_train = model.predict(X_train)
train_raw["Pred"] = pred_train
train_raw.to_csv('Train_MLP.csv', index=False)