...
"""
Accuracy :  0.4153414405986904 (on validation set)
"""
y_pred_lr_val = model_rf.predict(x_val)
...
y_pred_lr_test = model_rf.predict(x_test)
...
test_raw.to_csv('Test_RF.csv', index=False)

val_raw = pd.read_csv('Validate.csv')
y_pred_knn = model_rf.predict(x_val)
val_raw["Pred"] = y_pred_knn
val_raw.to_csv('Validate_RF.csv', index=False)
train_raw = pd.read_csv('Train.csv')
pred_train = model_rf.predict(x_train)
train_raw["Pred"] = pred_train
train_raw.to_csv('Train_RF.csv', index=False)